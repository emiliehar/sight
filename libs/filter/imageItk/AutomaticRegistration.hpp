...
#include "filter/imageItk/config.hpp"
#include "filter/imageItk/Metric.hpp"
 ...
 */
class FILTER_IMAGEITK_CLASS_API AutomaticRegistration
{
public:
     ...
     */
    FILTER_IMAGEITK_API void registerImage(
    ...
    FILTER_IMAGEITK_API void stopRegistration();

    /// Current metric evaluated by the optimizer.
    FILTER_IMAGEITK_API RealType getCurrentMetricValue() const;

    /// Current set of parameters used to evaluate the metric in the optimizer.
    FILTER_IMAGEITK_API const OptimizerType::ParametersType& getCurrentParameters() const;

    /// Gradient descent relaxation factor.
    FILTER_IMAGEITK_API RealType getRelaxationFactor() const;

    /// Gradient descent learning rate.
    FILTER_IMAGEITK_API RealType getLearningRate() const;

    /// Gradient magnitude tolerance.
    FILTER_IMAGEITK_API RealType getGradientMagnitudeTolerance() const;

    /// Current optimizer iteration.
    FILTER_IMAGEITK_API ::itk::SizeValueType getCurrentIteration() const;

    /// Current multi-resolution level.
    FILTER_IMAGEITK_API ::itk::SizeValueType getCurrentLevel() const;

    /// Current registration result.
    FILTER_IMAGEITK_API void getCurrentMatrix(const data::Matrix4::sptr& _trf) const;
