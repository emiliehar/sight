...
std::vector< ::cv::Vec2d> SUltrasoundImage::computeLineCircleIntersection(const cv::Vec2d& eql,
...
void SUltrasoundImage::process(const ::cv::Mat& input)
{
    ...
    ::cv::Mat elementDilate = ::cv::getStructuringElement(elementType,
...
SUltrasoundImage::ProbeSimulationSettings SUltrasoundImage::processConvexShape(::cv::Mat input,
                                                                               std::vector< ::cv::Vec2d> lines,
                                                                               ::cv::Mat thresh,
                                                                               ::cv::Mat binaryConvex)
{
    ...
    ::cv::ellipse2Poly(::cv::Point2d(center[0], center[1]), ::cv::Size2d(outerRadius,
                                                                         outerRadius), 0, startAngle, endAngle, 1,
                       arcPoints);
...
void SUltrasoundImage::updateBeamExtractionMap()
{
    ...
    for (int i = 0; i < mapSize.width; ++i)
    {
        ...
        for (int j = 0; j < mapSize.height; ++j)
        {
            ...
            m_extractionMap.at< ::cv::Vec2f >(j, i) = ::cv::Vec2f(static_cast<float>(posReal[0]),
