...
#include <filter/imageItk/MinMaxPropagation.hpp>
 ...
 * @code{.xml}
   <service type="sight::module::filter::imageItk::SPropagator">
 ...
 */
class MODULE_FILTER_IMAGEITK_CLASS_API SPropagator : public service::IOperator
{
public:
    ...
    MODULE_FILTER_IMAGEITK_API SPropagator();

    /// Destroys the service.
    MODULE_FILTER_IMAGEITK_API ~SPropagator() override;
...
protected:
    ...
    MODULE_FILTER_IMAGEITK_API void setBoolParameter(bool val, std::string key) override;

    /// Sets value. Key must be 'value'.
    MODULE_FILTER_IMAGEITK_API void setIntParameter(int val, std::string key) override;

    /// Sets radius. Key must be 'radius'.
    MODULE_FILTER_IMAGEITK_API void setDoubleParameter(double val, std::string key) override;

    /// Sets propagation mode. Key must be 'mode'.
    MODULE_FILTER_IMAGEITK_API void setEnumParameter(std::string val, std::string key) override;

private:

    typedef sight::filter::imageItk::MinMaxPropagation::CoordinatesType CoordinatesType;

    typedef sight::filter::imageItk::MinMaxPropagation::OrientationType OrientationType;
    ...
    sight::filter::imageItk::MinMaxPropagation::SeedsType convertDiffToSeeds() const;

    /// Line drawer.
    UPTR(sight::filter::image::LineDrawer) m_lineDrawer;

    /// Propagator.
    UPTR(sight::filter::imageItk::MinMaxPropagation) m_propagator;
    ...
    sight::filter::imageItk::MinMaxPropagation::Mode m_mode;
