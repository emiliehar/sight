...
public:
    ...
    inline void write()
    {
        ...
        auto root_object = data::Object::dynamicCast(m_session_writer->getObject());
        SIGHT_FATAL_IF("Root object is null or not a data object.", !root_object);

        // Create the session and serialize the root object
        auto session = detail::SessionSerializer::unique();
        session->serialize(m_session_writer->getFile(), root_object, m_password->getPassword());
    }
