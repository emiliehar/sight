...
void ArchiveTest::openTest()
{
...

    {
        ...
        // Read the stream
        std::unique_ptr<char> buffer = std::make_unique<char>(unencrypted_zstd_default.size());
        auto istream                 = archive_reader->openFile(unencrypted_zstd_default);
        istream->read(buffer.get(), static_cast<std::streamsize>(unencrypted_zstd_default.size()));

        std::string actual(buffer.get());
        actual.resize(unencrypted_zstd_default.size());

        CPPUNIT_ASSERT_EQUAL(unencrypted_zstd_default, actual);
    }
...

    {
        ...
        // Read the stream
        std::unique_ptr<char> buffer = std::make_unique<char>(encrypted_zstd_best.size());
        auto istream                 = archive_reader->openFile(encrypted_zstd_best, encrypted_zstd_best);
        istream->read(buffer.get(), static_cast<std::streamsize>(encrypted_zstd_best.size()));

        core::crypto::secure_string actual(buffer.get());
        actual.resize(encrypted_zstd_best.size());

        CPPUNIT_ASSERT_EQUAL(encrypted_zstd_best, actual);
    }
...

    {
        ...
        // Read the stream
        std::unique_ptr<char> buffer = std::make_unique<char>(unencrypted_deflate_fast.size());
        auto istream                 = archive_reader->openFile(unencrypted_deflate_fast);
        istream->read(buffer.get(), static_cast<std::streamsize>(unencrypted_deflate_fast.size()));

        std::string actual(buffer.get());
        actual.resize(unencrypted_deflate_fast.size());

        CPPUNIT_ASSERT_EQUAL(unencrypted_deflate_fast, actual);
    }
}
