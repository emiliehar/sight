...
#include "filter/imageItk/ItkImageCaster.hpp"
#include "filter/imageItk/Metric.hpp"
#include "filter/imageItk/Resampler.hpp"
...
struct RegistrationDispatch
{
    ...
    {
        filter::imageItk::MIPMatchingRegistration<PIXELTYPE>::registerImage(
            params.moving,
            params.fixed,
            params.transform
        );
...
)
{
    ...
    if(fixedVoxelVolume < movingVoxelVolume)
    {
        ...
        fixed = filter::imageItk::Resampler::resample(_fixed, inverseTransform, _moving->getSpacing2());
    }
    else
    {
        moving = filter::imageItk::Resampler::resample(_moving, _transform, _fixed->getSpacing2());
    }
